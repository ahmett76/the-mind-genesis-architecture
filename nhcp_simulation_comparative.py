...
class MindGenesisEcosystem:
    ...
    def check_cbp_trigger(self, current_drift, mode="tmga"):
        ...
        if mode == "avg":
            ...
            return False
        if len(self.drift_history) == 3:
            breaches = all(d > self.tau_drift for d in self.drift_history)
    ...
    def execute_arbitration(self, action_context, proposed_vectors, initial_weights,
                            conf_ethical, drift_val=0.01, stress_level=0.2, cbp_mode="tmga"):
        if self.check_cbp_trigger(drift_val, mode=cbp_mode):
...
def run_scenario_with_mode(scenario_name, cbp_mode, tau_drift=0.05, n_runs=100, seed=42):
    if seed is not None:
        random.seed(seed); np.random.seed(seed)
    from collections import Counter
    results = Counter()
    for i in range(n_runs):
        eco = MindGenesisEcosystem(tau_min=0.80, tau_drift=tau_drift)
        eco.drift_history = []
        ...
        if scenario_name == "ADVERSARIAL":
            for j in range(3):
                d = 0.15 * 0.3 if random.random() < 0.8 else 0.15
                _, status = eco.execute_arbitration(f"ADV_{j+1}", proposed_vectors,
                    initial_weights, conf_ethical=0.85, drift_val=d, cbp_mode=cbp_mode)
        elif scenario_name == "NOISY":
            for j in range(3):
                d = max(0.0, 0.12 + random.gauss(0, 0.12 * 0.20))
                _, status = eco.execute_arbitration(f"NOISY_{j+1}", proposed_vectors,
                    initial_weights, conf_ethical=0.85, drift_val=d, cbp_mode=cbp_mode)
            ...
            for cycle in range(3):
                ...
                _, status = eco.execute_arbitration(f"MA_{cycle+1}", proposed_vectors,
                    initial_weights, conf_ethical=0.85, drift_val=median_drift, cbp_mode=cbp_mode)
        results[status] += 1
    return results


def run_sensitivity(scenario_name, tau_values, n_runs=100, seed=42):
    results = {}
    for tau in tau_values:
        res = run_scenario_with_mode(scenario_name, "tmga", tau_drift=tau, n_runs=n_runs, seed=seed)
        results[tau] = res.get("CBP_CONTAINMENT", 0)
    return results


if __name__ == "__main__":
    print("=" * 70)
    print(" TMGA COMPARATIVE & SENSITIVITY ANALYSIS")
    print("=" * 70)

    all_results = {}

    print("\n--- 1. COMPARATIVE EVALUATION (100 runs each) ---")
    comparison_results = {}
    for scenario in ["ADVERSARIAL", "NOISY", "MULTI_AGENT"]:
        comparison_results[scenario] = {}
        print(f"\n  Scenario: {scenario}")
        for mode in ["tmga", "single", "avg"]:
            res = run_scenario_with_mode(scenario, mode, tau_drift=0.05, n_runs=100, seed=42)
            cbp = res.get("CBP_CONTAINMENT", 0)
            comparison_results[scenario][mode] = cbp
            print(f"    {mode:8s}: CBP={cbp:3d}/100")
    all_results["comparison"] = comparison_results

    print("\n--- 2. SENSITIVITY ANALYSIS (tau_drift) ---")
    tau_values = [0.03, 0.05, 0.07, 0.10, 0.15]
    sensitivity_results = {}
    for scenario in ["NOISY", "MULTI_AGENT"]:
        print(f"\n  Scenario: {scenario}")
        res = run_sensitivity(scenario, tau_values, n_runs=100, seed=42)
        sensitivity_results[scenario] = {str(k): v for k, v in res.items()}
